...
\nl \Begin{
    ...
    \nl $G \gets \FuncSty{createSupplementalGraphs}(\mathcal{P}, \mathcal{T})$ \label{line:buildsup}\;
    \nl $D \gets \FuncSty{init}(\operatorname{V}(\mathcal{P}), \operatorname{V}(\mathcal{T}), G)$
    ...
    \nl $\KwSty{return}~\FuncSty{search}(G, D) = \textnormal{Success}$ \label{line:search} \;
}
...
\begin{algorithm}[h]
\DontPrintSemicolon
\nl $\FuncSty{createSupplementalGraphs}$ (Graph $\mathcal{P}$, Graph $\mathcal{T}$) $\rightarrow$ GraphPairs \;
\nl \Begin{
    \nl $\begin{aligned}\KwSty{return}~\Big\{
            &(\mathcal{P},\ \mathcal{T}),
            &&(\mathcal{P}^{\left[2,1\right]},\ \mathcal{T}^{\left[2,1\right]}),
            &&(\mathcal{P}^{\left[2,2\right]},\ \mathcal{T}^{\left[2,2\right]}),
            &&(\mathcal{P}^{\left[2,3\right]},\ \mathcal{T}^{\left[2,3\right]}), \\
            &&&(\mathcal{P}^{\left[3,1\right]},\ \mathcal{T}^{\left[3,1\right]}),
...
\nl \Begin{
    \nl \Repeat{\nl$R$ \textnormal{is unchanged}}{
        \nl \ForEach{$v \in V$}{
            \nl $D_v \gets \bigcap_{(P,\,T) \in G} \left\{ w \in R :~
...
\nl $\FuncSty{search}$ (GraphPairs $G$, Domains $D$) $\rightarrow$ Fail $F$ \KwSty{or} Success \;
\nl \Begin{
    ...
    \nl \ForEach {$v' \in D_v~\textnormal{ordered by static degree in}~\mathcal{T}$\label{line:parforeach}}{
        ...
        \nl \Switch{$\FuncSty{assign}(G, D', v, v')$\label{line:assign}}{
            \nl \lCase{\textnormal{Fail $F'$~\KwSty{then}}}{$F \gets F \cup F'$\label{line:fassign}}
            \nl \Case{\textnormal{Success~\KwSty{then}}}{
                \nl \Switch{$\FuncSty{search}(G, D' - D_v)$\label{line:recursesearch}}{
                    ...
                    \nl \Case{\textnormal{Fail $F'$~\KwSty{then}}\label{line:searchfailed}}{
                        ...
                        \nl $\KwSty{return}~\textnormal{Fail}~F'$\label{line:backjump}}
...
\begin{algorithm}[h]
\DontPrintSemicolon
\nl $\FuncSty{assign}$ (GraphPairs $G$, Domains $D$, Vertex $v$, Vertex $v'$) $\rightarrow$ Fail $F$ \KwSty{or} Success \;
\nl \Begin{
